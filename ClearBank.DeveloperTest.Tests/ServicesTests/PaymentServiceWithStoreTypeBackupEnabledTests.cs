...
public class PaymentServiceWithStoreTypeBackupEnabledTests
{
    ...
    public void MakePayment_ForRequest_ReturnsSuccess_True(PaymentScheme requestedPaymentScheme)
    {
        ...
        mockBackupAccountDataStore.Verify(x => x.UpdateAccount(It.IsAny<Account>()), Times.Once);
    }
    ...
    public void MakePayment_ForRequest_ReturnsSuccess_True_WithCorrectUpdatedBalance(PaymentScheme requestedPaymentScheme)
    {
        ...
        Assert.True(result.Success);
        account.Balance -= paymentRequest.Amount;
        mockBackupAccountDataStore.Verify(x => x.UpdateAccount(It.Is<Account>(x => x == account)), Times.Once);
    }

    [Fact]
    public void MakePayment_ForRequest_ReturnsSuccess_True_ShouldReturnEarlyOnBackupStoreEnabled()
    {
        var paymentRequest = fixture.Create<MakePaymentRequest>();
